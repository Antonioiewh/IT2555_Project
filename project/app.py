...
def upload_message_attachment():
    ...
    try:
        # --- 1. NEW: VIRUS SCANNING (ClamAV) ---
        print(f"Scanning {file.filename} with ClamAV...")
        is_safe_av, threat_msg = av_scanner.scan_stream(file)
        file.seek(0) # Reset pointer after scan
        
        if not is_safe_av:
            print(f"SECURITY ALERT: {threat_msg}")
            # Returning 406 triggers the WhatsApp-style error bubble on the frontend
            return jsonify({
                'ok': False, 
                'error': f"Malware detected: {threat_msg}"
            }), 406
        ...
        if not validation_result['is_safe']:
            ...
            # Return 406 here as well so the frontend shows the red failure bubble!
            return jsonify(ok=False, error=f'Security validation failed: {threats_msg}'), 406
        ...
        # Determine file kind (THIS IS WHAT FIXES THE ICON VS IMAGE ISSUE)
        ...
        return jsonify(
            ok=True,
            url=url_for('static', filename=rel_path), # This ensures the correct absolute path is sent to JS
            name=filename,
            size=file_size,
            mime=mime_type,
            kind=kind
