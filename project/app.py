...
socketio = SocketIO(app, cors_allowed_origins="*", message_queue='redis://redis:6379', logger=True, engineio_logger=True)   
...
def handle_join_caht(data):
    print(f"User {current_user.user_id} joined chat {data['chat_id']}") #debug 
    chat_id = data['chat_id']
    join_room(str(chat_id))

@socketio.on('send_message')
def handle_send_message(data):
    print(f"User {current_user.user_id} sent message to chat {data['chat_id']}") #debug
    chat_id = data['chat_id']
    encrypted_message = data['message'] # Assuming the message is encrypted
